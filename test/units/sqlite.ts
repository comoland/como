...
test("sql basics", async () => {
    ...
    const date =new Date();
    const obj = await db.exec('INSERT INTO place (country, telcode, createdAt) VALUES (?, ?, ?)', "Hong Kong", 9.9, date.toISOString());
    const result = await db.query("SELECT id, country, telcode, city, createdAt FROM place WHERE id=? LIMIT ?", obj.lastInsertId, 10);
...
test("sql trans", async () => {
    ...
    while(i++ < 1000) {
        ...
        `, "Hong Kong", 99, (new Date()).toISOString());
    }
